#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 24-Bus_Power_System_Step_1_24_Bus ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Step_1_24_Bus = Model(HiGHS.Optimizer)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
@constraint(Step_1_24_Bus, conventional_max_generation[t=1:T, cg=1:CG], p_g[t,cg] <= C_p_max[cg])

# The power generated from convetional generators cg must be equal or more that their minimum capacity generation limid
@constraint(Step_1_24_Bus, conventional_min_generation[t=1:T, cg = 1:CG], p_g[t,cg] >= C_p_min[cg])

# The power generated from wind parks wg can not exceed their maximum capacity generation limit
@constraint(Step_1_24_Bus, wind_park_generation[t = 1:T, wg = 1:WG], w_g[t,wg] <= W_p[t,wg])
...
@constraint(Step_1_24_Bus, ramp_up[t=2:T, cg = 1:CG], p_g[t,cg] - p_g[t-1,cg] <= Ramp_up[cg])
...
@constraint(Step_1_24_Bus, ramp_down[t=2:T, cg = 1:CG], p_g[t,cg] - p_g[t-1,cg] >= -Ramp_down[cg])
...
@constraint(Step_1_24_Bus, ramp_up_initial[t = 1, cg = 1:CG], p_g[cg,t]-Initial_State[cg] <= Ramp_up[cg])
...
@constraint(Step_1_24_Bus, ramp_down_initial[t = 1, cg = 1:CG], p_g[cg,t]-Initial_State[cg] >= -Ramp_down[cg])

# The power generated from all generators in hour t can not exceed the load of demand points DP
@constraint(Step_1_24_Bus, generation_demand_points[t = 1:T, dp = 1:DP], p_d[t,dp] <= Demand[t,dp])
...
@constraint(Step_1_24_Bus, balance_equation[t = 1:T],
...
@constraint(Step_1_24_Bus, state_of_charge, Eb_s[1] == eta_storage * p_b[1]) # State of charge in first time step equal to the energy stored multiplied with the efficiency of the battery

@constraint(Step_1_24_Bus, power_of_battery[t=2:T], Eb_s[t] == Eb_s[t-1] + eta_storage * p_b[t]) # State of charge equality

@constraint(Step_1_24_Bus, limits_of_battery_Up[t=1:T], p_b[t] <= cap_storage_max) # Upper bound limit of battery charge

@constraint(Step_1_24_Bus, limits_of_battery_Low[t=1:T], p_b[t] >= cap_storage_min) # Lower bound limit of battery charge

@constraint(Step_1_24_Bus, max_energy_stored[t=1:T], Eb_s[t] <= battery_capacity) # Maximum potential energy stored in battery can not exceed battery's capacity
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Solve Model
optimize!(Step_1_24_Bus)
println("")
println("Termination status: $(termination_status(Step_1_24_Bus))")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Print Solutions
println("----------------------------------------------------");
if termination_status(Step_1_24_Bus) == MOI.OPTIMAL
    println("Maximum Social Welfare(Optimal objective value): $(objective_value(Step_1_24_Bus))")
